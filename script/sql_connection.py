...
# ----------------------------------------------------------------------------------------------------------------------
# Drop a column
# conn.execute(sa_text('''ALTER TABLE %s DROP COLUMN hora1''' % table).execution_options(autocommit=True))
...
# conn.execute(sa_text('''ALTER TABLE %s ADD hora INTEGER ''' % table).execution_options(autocommit=True))
...
pd_sql.to_csv('test1.csv', index=False)
